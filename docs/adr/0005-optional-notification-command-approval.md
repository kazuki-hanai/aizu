...
3. When the setting is on, one canonical local `Bash` request may show a large, centered Aizu
   ...
   always on top, receives focus, temporarily takes visual priority over queued passive banners,
   and cannot be closed or swiped away. The command becomes actionable only after native window
   presentation and frontend render acknowledgement. The decision is consumed once.

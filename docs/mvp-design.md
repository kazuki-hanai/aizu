...
通知本文はロック画面などに表示される可能性があるため保守的に扱うが、agent の
メッセージが「出ないケース」をなくすことを優先し、first-party adapter のメッセージは
既定で通知に表示する。
...
- 既定は app-owned Aizu Banner とし、macOS 通知権限を要求しない。app 稼働中は右上へ最大3件を表示し、privacy filter 後の本文を省略せず、CommonMark と GitHub Flavored Markdown の段落、改行、見出し、リスト、引用、強調、table、inline code、fenced code block をcompactに表示する。raw HTMLは実行せず、linkとimageはnetwork/navigationを発生させない非対話表示にする。通常通知の表示先は`notification_display`でprimary display（既定）、最初に接続されたsecondary display、keyboard focusを持つwindowのdisplay、pointer displayから選ぶ。secondary displayが存在しない場合、または選択先を取得・照合できない場合はprimary displayへfallbackする。banner windowがdisplayの利用可能高へ達した場合、外側viewportを縦scroll可能にし、本文や後続通知を切り捨てない。自然content高の計測領域はscroll viewportから分離し、通知削除時にはwindowを縮小できるようにする。ユーザーが右上の close button または横スワイプで閉じるまで自動消去しない。短いドラッグや縦操作では閉じず、本文とcodeは選択可能とする。§13.5 のlocal approvalは独立した`approval_display`を使う専用dialogとし、既定では中央、設定で同display右上へ切替可能とし、closeとswipeを提供しない。復帰情報を持つ local first-party 通知の通常クリック/Enter/Spaceだけは terminal 復帰を行い、本文選択中や swipe 後の合成 click は復帰させない。クリックで Aizu main window は開かない。新規表示とスワイプ終了には短い animation を使い、OS の reduced-motion 設定時は無効化する。超過分および app 再起動後は Recent activity を確認経路とする。
...
- 設定オン時だけ、canonical `Bash` toolのbounded exact command、またはClaude Code `WebFetch` toolのbounded exact HTTP(S) URLをprivate Unix-domain socket経由で大型Aizu approval dialogへ一時表示する。WebFetchの`prompt`、cwd、transcript、session情報はbroker requestへ含めず、URLは選択可能なtextとして表示してlink化しない。表示先は通常通知と独立した`approval_display`を使う。`center_approval_dialogs` は既定オンで選択display中央へ配置し、オフでは同display右上へ配置する。dialogは常時前面とし、native focusを要求して未focus時は一回限りのinformational user-attentionへfallbackする。queued passive bannerを削除せず一時的に隠し、`Deny` / `Allow once` だけを提供する。closeとswipeは提供せず、native window showとfrontend render ackの両方が確認できたrequestだけを一回限り消費する
...
- raw commandまたはWebFetch URLはhook process、private socket buffer、desktop memory、banner WebViewにだけ一時保持し、spool、desktop DB、history、notification outbox、settings、log、SSH bridge、macOS Notification Centerへ保存・転送しない。banner以外のWebViewからraw target取得・ack・decision・dismissを拒否し、frontend event emit権限も与えない

...
- 既定は app-owned Aizu Banner とし、macOS 通知権限を要求しない。app 稼働中は右上へ最大3件を表示し、privacy filter 後の本文を省略せず、ユーザーが閉じるか開くまで自動消去しない。超過分および app 再起動後は Recent activity を確認経路とする。
